...
export function NeonUploadManager({ sections }: { sections: NeonAdminSection[] }) {
  ...
  async function submit(path: string) {
    ...
    try {
      ...
      const payload = (await response.json()) as UploadPreview | ImportResult | UploadErrorPayload;

      if (!response.ok) {
        const errorPayload = payload as UploadErrorPayload;
        setError(errorPayload.error || "Upload request failed.");

        if (errorPayload.preview) {
          setPreview(errorPayload.preview);
        } else if (errorPayload.errors) {
          setPreview({
            targetTable,
            totalRows: 0,
            validRows: 0,
            invalidRows: errorPayload.errors.length,
            headers: [],
      ...
      if (path.endsWith("/preview")) {
        setPreview(payload as UploadPreview);
      } else {
  ...
  return (
    <section className="admin-panel rounded-[1.5rem] p-5">
      ...
      <div className="mt-5 flex flex-wrap gap-3 border-t border-[color:var(--border)] pt-5">
        <Link
          href="/members/admin/uploads/templates/pdf"
          className="inline-flex h-10 items-center gap-2 rounded-full border border-[color:var(--border-strong)] px-5 text-sm font-semibold text-white hover:text-[color:var(--accent)]"
        >
        ...
        {targetTable ? (
          <Link
            href={`/members/admin/uploads/templates/${targetTable}`}
            className="inline-flex h-10 items-center gap-2 rounded-full border border-[color:var(--border-strong)] px-5 text-sm font-semibold text-white hover:text-[color:var(--accent)]"
          >
      ...
      {result ? (
        <div className="mt-5 rounded-xl border border-[rgba(24,213,141,0.24)] bg-[rgba(22,135,91,0.18)] p-4 text-sm text-[color:var(--success)]">
          ...
          {result.errors && result.errors.length > 0 ? (
            <ul className="mt-3 max-h-72 space-y-2 overflow-auto text-[#f5c26b]">
