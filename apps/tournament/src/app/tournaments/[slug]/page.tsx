...
export default async function TournamentPage({
  ...
  const selectedTeam = teamParam ? (teams.find((t) => t.id === teamParam) ?? null) : null;
  const displayFixtures = selectedTeam
    ? publicFixtures.filter((f) => f.homeTeamId === selectedTeam.id || f.awayTeamId === selectedTeam.id)
    : publicFixtures;
  const completedFixtures = displayFixtures.filter(isFixtureComplete);
  const results = [...completedFixtures]
    .sort((a, b) => new Date(b.startsAt).getTime() - new Date(a.startsAt).getTime())
    .slice(0, 4);
  const nextFixture = displayFixtures.find((fixture) => !isFixtureComplete(fixture));
      ...
      <section className="mx-auto grid w-full max-w-6xl gap-5 overflow-hidden px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,1.35fr)_380px]">
        <div className="min-w-0 space-y-5">
          ...
          {placementFixtures.length > 0 ? (
            <PlayoffPanel bracket={[]} placementFixtures={displayPlacementFixtures} teams={teams} />
          ) : null}
          ...
          <section id="schedule" className="scroll-mt-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {event.schedulePublished && teams.length > 0 ? (
              <TeamFilter selectedTeamId={selectedTeam?.id} slug={event.slug} teams={teams} />
            ) : null}
            {event.schedulePublished ? (
              <FullDaySchedule
                bracket={placementFixtures.length > 0 ? [] : (selectedTeam ? [] : publicBracket)}
  ...
  return (
    <div className="mt-4 flex max-w-full flex-wrap gap-2">
      ...
      {teams.map((team) => (
        <Link
          key={team.id}
          href={`/tournaments/${slug}?team=${team.id}`}
          className={`inline-flex h-8 items-center rounded-full px-3 text-xs font-bold transition ${
