...
function getScheduleFairnessWarnings(fixtures: Fixture[], teams: Team[]) {
  ...
  // Pitch conflicts: two games on the same pitch at the same time
  const pitchSlots = new Map<string, Fixture>();
  for (const fixture of fixtures) {
    const key = `${fixture.pitch}:${fixture.startsAt}`;
    const conflict = pitchSlots.get(key);
    if (conflict) {
      warnings.push(
        `Pitch conflict on ${fixture.pitch} at ${formatTime(fixture.startsAt)}: ` +
        `${teamName(teams, conflict.homeTeamId)} vs ${teamName(teams, conflict.awayTeamId)} ` +
        `and ${teamName(teams, fixture.homeTeamId)} vs ${teamName(teams, fixture.awayTeamId)}.`
      );
    } else {
      pitchSlots.set(key, fixture);
    }
  }
