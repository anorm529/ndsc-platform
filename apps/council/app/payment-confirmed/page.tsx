...
export default async function PaymentConfirmedPage({
  ...
  const formattedAmount = displayAmount !== null
    ? new Intl.NumberFormat("en-GB", { style: "currency", currency: "GBP" }).format(displayAmount)
    : null;
      ...
      <div className="w-full max-w-md rounded-2xl border border-[rgba(16,185,129,0.2)] bg-white p-10 text-center shadow-sm">
        ...
        {displayName && formattedAmount ? (
          <p className="text-[0.88rem] text-slate-500">
            Thank you{displayName ? `, ${displayName}` : ""} — your payment of{" "}
            <span className="font-semibold text-slate-700">{formattedAmount}</span> has been recorded.
