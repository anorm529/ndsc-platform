      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <p className="text-[0.9rem] text-[color:var(--muted-foreground)]">
            <span className="font-semibold text-slate-800">{visibleMembers.length}</span> active members
            {isCapt ? " on your team" : ""}
          </p>
        </div>
        {isCapt ? (
        ...
        return (
          <section key={teamId} className="council-panel rounded-2xl border p-5">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="text-[0.92rem] font-semibold text-slate-800">
            ...
            <ul className="divide-y divide-[color:var(--border)]">
              {members.map((m) => (
                <li key={m.id} className="flex items-center gap-3 py-2.5">
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[rgba(20,184,166,0.08)] text-[0.8rem] font-semibold text-[color:var(--accent)]">
                    {memberDisplayName(m).charAt(0).toUpperCase()}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[0.88rem] font-medium text-slate-800">
                      {memberDisplayName(m)}
                    </p>
                  ...
                  {m.playerId ? (
                    <span className="shrink-0 rounded-full bg-[rgba(16,185,129,0.1)] px-2 py-0.5 text-[0.65rem] text-[color:var(--success)]">
                      Linked
                    </span>
                  ) : null}
                </li>
