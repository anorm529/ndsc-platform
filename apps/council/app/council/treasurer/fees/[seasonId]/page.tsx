...
export default async function SeasonFeesPage({ params }: { params: Promise<{ seasonId: string }> }) {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <Link href="/council/treasurer/fees" className="flex items-center gap-2 text-[0.82rem] text-[color:var(--muted-foreground)] hover:text-white">
      ...
      <div className="council-panel rounded-2xl border p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-[1.1rem] font-semibold text-white">{season.label}</h2>
            {season.dueDate ? (
          ...
          {season.isActive ? (
            <span className="flex items-center gap-1 rounded-full bg-[rgba(24,213,141,0.12)] px-2.5 py-1 text-[0.7rem] text-[color:var(--success)]">
      ...
      {unenrolled.length > 0 ? (
        <section className="council-panel rounded-2xl border p-5">
          ...
          <form action={handleAddPlayer} className="grid gap-3 sm:grid-cols-3">
            ...
            <button
              type="submit"
              className="sm:col-span-3 rounded-xl bg-[linear-gradient(180deg,#3b837d_0%,#31756e_100%)] py-2.5 text-[0.85rem] font-medium text-[#04101a] hover:brightness-105"
            >
              Add player
            </button>
          </form>
      ...
      {playerFees.length === 0 ? (
        ...
        <section className="council-panel rounded-2xl border p-5">
          <h3 className="mb-4 text-[0.88rem] font-semibold text-white">Player fees</h3>
              ...
              return (
                <div key={pf.id} className={[
                  ...
                  fullyPaid
                    ...
                    : "border-[color:var(--border)] bg-[rgba(10,24,41,0.4)]",
                ].join(" ")}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-[0.88rem] font-medium text-white">{pf.playerName}</p>
                        <p className={[
                      ...
                      {!fullyPaid ? (
                        <form action={handleRecordPayment} className="mt-3 flex flex-wrap items-center gap-2">
                          ...
                          <input
                            ...
                            placeholder="Amount"
                            required
                            className="w-24 rounded-lg border border-[color:var(--border)] bg-[rgba(10,24,41,0.8)] px-2.5 py-1.5 text-[0.78rem] text-white outline-none focus:border-[color:var(--border-strong)]"
                          />
                          <select
                            name="paymentMethod"
                            className="rounded-lg border border-[color:var(--border)] bg-[rgba(10,24,41,0.8)] px-2.5 py-1.5 text-[0.78rem] text-white outline-none"
                          >
                          ...
                          <input
                            name="reference"
                            placeholder="Ref"
                            className="w-24 rounded-lg border border-[color:var(--border)] bg-[rgba(10,24,41,0.8)] px-2.5 py-1.5 text-[0.78rem] text-white outline-none placeholder:text-[#4a5568] focus:border-[color:var(--border-strong)]"
                          />
                          <button
                            type="submit"
