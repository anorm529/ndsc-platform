          ...
          <nav className="flex gap-2 overflow-x-auto themed-scrollbar">
            {links.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="shrink-0 rounded-xl border border-[#2B4162] bg-slate-700/40 px-3 py-2 text-sm font-semibold text-slate-200 transition hover:bg-slate-700 hover:text-white"
              >
                {link.label}
              </Link>
            ))}
